...
if (strlen($_SESSION['tsasaid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $eid = $_GET['editid'];
        $sql = "UPDATE tblteacher SET FirstName=?, LastName=?, MobileNumber=?, Email=?, Gender=?, Dob=?, CourseID=?, Religion=?, Address=? WHERE ID=?";
        $stmt = $dbh->prepare($sql);
        $stmt->bind_param("ssssssisss", $fname, $lname, $mobnum, $email, $gender, $dob, $cid, $religion, $address, $eid);
        $stmt->execute();
                ...
                <div class="card alert">
                    <div class="card-body">
                        <form name="" method="post" action="" enctype="multipart/form-data">
                            <?php
                            $eid = $_GET['editid'];
                            $sql = "SELECT tblcourse.ID as cid, tblcourse.BranchName, tblcourse.CourseName, tblteacher.ID, tblteacher.EmpID, tblteacher.FirstName, tblteacher.LastName, tblteacher.MobileNumber, tblteacher.Email, tblteacher.Gender, tblteacher.Dob, tblteacher.CourseID, tblteacher.Religion, tblteacher.Address, tblteacher.Password, tblteacher.ProfilePic, tblteacher.JoiningDate FROM tblteacher JOIN tblcourse ON tblcourse.ID=tblteacher.CourseID WHERE tblteacher.ID=?";
                            $stmt = $dbh->prepare($sql);
